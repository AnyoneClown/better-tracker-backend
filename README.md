...
The current application is intentionally **single-user and has no
authentication or authorization**. Every caller can read and change every row.
Run one trusted instance per user, and do not expose it to an untrusted network
without adding authentication and ownership checks.
...
curl -fsS "$BASE_URL/api/v1/workouts?offset=0&limit=20"
...
curl -fsS "$BASE_URL/api/v1/finance/summary?year=2026&month=7&currency=USD"
```
...
credentials, restricted CORS origins, and application authentication. Replace
the local URL with the secure `cockroachdb+asyncpg://` connection details for
that cluster.
